...
class GRUModel(Model):
    ...
    def gen_model(self):
        model = Sequential()
        model.add(GRU(50,return_sequences=True,activation=self.activation,recurrent_activation=self.recurrent_activation))
        model.add(GRU(50,return_sequences=True,activation=self.activation,recurrent_activation=self.recurrent_activation))
        model.add(GRU(50,activation=self.activation,recurrent_activation=self.recurrent_activation))
        model.add(Dense(1))
...
if __name__ == "__main__":
    ...
    print('testing')
    test = Test(Model=GRUModel, params=params, tests=rolling_window_tests, f='gru-rolling-tests.json', plot=True)
