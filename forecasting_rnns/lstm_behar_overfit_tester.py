...
class LSTM_Behar(Model):
    ...
    def predict(self, test_data, rolling_window_test=False):
        ...
        for i in range(len(test_data)):
            prediction = self.model.predict(observed.reshape(1, self.d, len(self.train_columns)))
            labels.append(scale_train_obs[i, self.label_column_index])
...
if __name__ == "__main__":
    ...
    test = Test(Model=LSTM_Behar, params=params, tests=heavy_hitters_tests, f='Behar-Std-250-Close-Fixed-OverfitTests.json', plot=True)
