...
for ticker,_ in zip(training.keys(), testing.keys()):
    ...
    model, _ = train_predictor.train(model, training_data, moghar.label_column_index)
    preds, actual = train_predictor.predict(model, back_test_data, moghar.label_column_index)
...
output_file = open('moghar_fastquant_results.json', 'w')
